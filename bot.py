...
# Токен берётся из переменной окружения BOT_TOKEN (задаётся в настройках Render)
...
async def start_handler(message: Message):
    await message.answer(
        ...
        "И я покажу модель, фон, узор и текущего владельца."
    )
...
async def nft_link_handler(message: Message):
    ...
    if not info.raw_description:
        await status.edit_text(
            "Страница получена, но не удалось распознать поля. "
            "Возможно, формат отличается — пришли скриншот HTML, поправлю парсер."
        )
        return
    ...
    if info.model:
        lines.append(f"<b>Модель:</b> {info.model} <i>{info.model_pct}%</i>")
    if info.backdrop:
        lines.append(f"<b>Фон:</b> {info.backdrop} <i>{info.backdrop_pct}%</i>")
    if info.pattern:
        lines.append(f"<b>Узор:</b> {info.pattern} <i>{info.pattern_pct}%</i>")
    if info.owner:
        lines.append("")
...
async def run_dummy_webserver():
    """
    Render (бесплатный Web Service) требует, чтобы приложение слушало порт,
    иначе считает деплой неудачным. Поднимаем простейший HTTP-ответчик
